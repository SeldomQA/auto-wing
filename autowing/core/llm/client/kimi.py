...
class KimiClient(BaseLLMClient):
    """
    A client for interacting with the Kimi (Moonshot AI) model.
    Kimi exposes an OpenAI-compatible API, so this client reuses the
    OpenAI SDK against the Moonshot endpoint.
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        Args:
            api_key (Optional[str]): The API key for Kimi. If not provided,
                                   will try to get from KIMI_API_KEY (or the
                                   official MOONSHOT_API_KEY) environment variable.
        ...
        if not self.api_key:
            raise ValueError("Kimi API key is required (set KIMI_API_KEY or MOONSHOT_API_KEY)")

        self.base_url = os.getenv("KIMI_BASE_URL", "https://api.moonshot.cn/v1")
        self.model_name = os.getenv("KIMI_MODEL_NAME", "kimi-latest")
    ...
    def _format_messages(self, prompt: str, context: Optional[Dict[str, Any]] = None) -> List[Dict[str, str]]:
        """
        Format messages for the Kimi API.
    ...
    def complete(self, prompt: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        Raises:
            Exception: If there's an error communicating with the Kimi API
        """
        try:
            ...
            raise Exception(f"Kimi API error: {str(e)}") from e

    def complete_with_vision(self, prompt: Dict[str, Any]) -> str:
        """
        Send a vision-based completion request to the Kimi API.
